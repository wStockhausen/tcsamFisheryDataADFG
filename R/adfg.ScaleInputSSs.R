#'
#' @title Calculate the "input" sample sizes to TCSAM02 for ADFG size composition data
#'
#' @description Function to calculate the "input" sample sizes to TCSAM02 for ADFG size composition data.
#'
...
#' @param ss_max - maximum input sample size (summed across all non-year factors) allowed
#'
...
#' @export
#'
adfg.ScaleInputSSs<-function(dfrSS,
                             ss_scl,
                             ss_max=200){
  ...
  dfrSSp<-dfrSS %>%
           ...
           dplyr::group_by(year) %>%
           dplyr::mutate(ss_scl=min(ss_max,ss_max*(ss_tot/ss_scl))) %>%
           dplyr::ungroup();
  #--apportion scaled sample sizes by year, sex and other factors
  dfrSSs_inp<-dfrSSp %>%
                 dplyr::inner_join(dfrSS,by=c("year")) %>%
